...
if __name__ == '__main__':
    ...
    run_clustering.run(data, labels, df, dataset_name="Vowel", k=11, num_features=[12], plot_3D=True)

    df, meta = read_arff_files.main('iris.arff')
    data, labels = iris_pre_processing.main(df)
    run_clustering.run(data, labels, df, dataset_name="Iris", k=3, num_features=[2], plot_3D=False)

    df, meta = read_arff_files.main('pima_diabetes.arff')
    data, labels = pima_diabetes_pre_processing.main(df)
    run_clustering.run(data, labels, df, dataset_name="Pima Diabetes", k=4, num_features=[5], plot_3D=True)
